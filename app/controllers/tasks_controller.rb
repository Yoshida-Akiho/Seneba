class TasksController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @task.save
        format.html { redirect_to @task, notice: '登録完了しました' }
  ...
  def update
    respond_to do |format|
      if @task.update(task_params)
        format.html { redirect_to @task, notice: '変更完了しました' }
  ...
  def destroy
    @task.destroy
    respond_to do |format|
      format.html { redirect_to tasks_url, notice: '削除完了しました' }
  ...
  private
    ...
    def task_params
      params.require(:task).permit(:due_date, :category_id, :name, :dune, :user_id)
    end
end
